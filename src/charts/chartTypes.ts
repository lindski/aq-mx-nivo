...
/**
 * Whether the chart type reads a `features` collection out of the CONFIGURATION, and throws without
 * one.
 *
 * This is not the same question as `CHART_DATA_SHAPE`, which is about the `data` prop — `GeoMap` is
 * `"features"` there because it takes no meaningful data at all, while `Choropleth` is `"array"`
 * because it takes both. Both nonetheless need `features` in the configuration, and neither Nivo
 * component defaults it: `GeoMap.js` and `Choropleth.js` each destructure `features` bare, and
 * `features.map(...)` then throws several frames below anything named after this widget.
 *
 * **Why the widget guards rather than letting the boundary catch it.** It is caught — but React logs
 * every error a boundary catches, from inside React, and a wrapper cannot suppress that. So the only
 * way to keep a consumer's console clean is to not let the throw happen. It is a real scenario
 * rather than a theoretical one: a page that lets the user pick the chart type re-renders with the
 * NEW type and the OLD configuration for one commit, and if that configuration came from a Bar chart
 * it has no `features`.
 *
 * Measured, not assumed: switching through all 26 types in the gallery playground on 2026-09-08
 * produced exactly one throw, on `GeoMap`. `Choropleth` is here on the strength of its source rather
 * than an observation — the same bare destructure and the same `features.map` in `useChoropleth` —
 * because being one render-timing away from the identical crash is not a reason to leave it out.
 */
export const CHART_NEEDS_FEATURES: Record<ChartType, boolean> = {
    AreaBump: false,
    Bar: false,
    Bullet: false,
    Bump: false,
...
/**
 * Whether this chart cannot be drawn because its configuration carries no `features`.
 *
 * Deliberately strict about what counts. An empty array is as fatal as a missing key for the purpose
 * this serves — a map of no countries is not a map — and a `features` that is not an array reaches
 * the same `.map` and throws the same way, so the shape is checked rather than assumed.
 */
export function isMissingFeatures(chartType: ChartType, configuration: Record<string, unknown>): boolean {
    if (!CHART_NEEDS_FEATURES[chartType]) {
        return false;
    }
